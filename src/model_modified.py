...
class GPTWithCoT(nn.Module):
    ...
    def forward(self, obs, states, timesteps, actions=None, key_state_mask=None):
        ...
        B, T, C, H, W = obs.shape
        if C == 4:
            rgb = obs[:,:,0:3,:,:].view(-1,3,H,W)
            depth = obs[:,:,3,:,:].view(-1,1,H,W).repeat(1, 3, 1, 1)
            #[B*T, 3, 16, 16]
            # pdb.set_trace()
            embedd = self.dual_encoder_1(rgb, depth)
            #[B*T, 3, 16, 16] -> [B, T, 768]
            dual_embedd = embedd.view(B, T, -1)
            state_embeddings = self.state_encoder(states)
            state_embeddings = torch.cat((state_embeddings, dual_embedd), dim=2)
        else:
