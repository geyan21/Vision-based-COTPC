...
class MS2Demos(Dataset):
    ...
    def __getitem__(self, index):
        ...
        rgbds = self.data['obs'][index][s_idx:e_idx]
        T, H, W, C = rgbds.shape
        if C==4:
            # T H W 4
            rgb = torch.stack([self.img_transform(Image.fromarray(rgbd[:,:,0:3])) for rgbd in rgbds])
        ...
        data_dict = {
            'o': rgbd_transformed.astype(np.float32),  # rgbd image T 8 H W or T 4 H W
    ...
    def convert_observation(self, observation):
        ...
        if self.task=='PushChair-v1' or self.task=='PushChair-v2':
            ...
            rgbd = np.concatenate([rgb, depth], axis=-1).astype(np.uint8)
            return rgbd
        elif self.task=='TurnFaucet-v0' or self.task=='TurnFaucet-v1' or self.task=='TurnFaucet-v2' or self.task=='StackCube-v0' or self.task=='LiftCube-v0':
            ...
            # combine the RGB and depth images
            rgbd = np.concatenate([rgb1, depth1, rgb2, depth2], axis=-1).astype(np.uint8)
            return rgbd
        elif self.task=='PegInsertionSide-v0':
    ...
    def convert_state(self, observation):
        ...
        if self.task=='PushChair-v1' or self.task=='PushChair-v2':
            base_ang_vel = np.array(observation['agent']['base_ang_vel'])[:,None]
            base_orientation = np.array(observation['agent']['base_orientation'])[:,None]
