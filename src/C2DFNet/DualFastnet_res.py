...
class DualFastnet(nn.Module):
    def __init__(self, channel=16):  # ,down_factor=4
        ...
        self.translayer = DenseTransLayer(channel, channel)
        # 动态卷积融合

        self.selfdc = SDFM(channel, channel, channel, 3, down_factor=8)
        self.decoder_plus_rgb = DFM(channels=channel)
        self.decoder_plus_depth = DFM(channels=channel)
        # transfor
        ...
        self.fuse_final = BasicConv_PRelu(16, 3, 1)
        self.fuse_final2 = BasicConv_PRelu(16, 3, 1, 2)
    ...
    def forward(self,rgb,depth):
        ...
        output = rgb_high_feature_dy
        # output = self.downsample(output)
        # output = self.fuse_final(output)
        output = self.fuse_final2(output)
...
if __name__=="__main__":
    ...
    a = torch.zeros(1, 3, 128, 128).cuda()
